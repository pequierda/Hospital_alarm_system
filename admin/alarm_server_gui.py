...
# Import fcntl only on Unix-like systems (not Windows)
...
class AlarmServerGUI:
    def __init__(self, host='10.0.10.13', port=9999):
        ...
        # Setup system tray after GUI is ready (optional)
    ...
    def check_single_instance(self):
        ...
        lock_file_path = "alarm_server.lock"
        
        try:
            # Try to create and lock the file
            self.lock_file = open(lock_file_path, 'w')
            
            # On Windows, we can't use fcntl, so we'll use a different approach
            if sys.platform == "win32":
                # Try to acquire exclusive access to the file
                import msvcrt
            ...
            try:
                ...
                with open(lock_file_path, 'r') as f:
                    pid = f.read().strip()
                print(f"Another instance of Alarm Server is already running (PID: {pid})")
            except:
                print("Another instance of Alarm Server is already running")
            
            print("Only one instance of the Alarm Server can run at a time.")
            ...
            try:
                ...
                messagebox.showerror("Instance Already Running", 
                                   "Another instance of Alarm Server is already running!\n\n"
    ...
    def cleanup_lock_file(self):
        ...
        if self.lock_file:
            try:
                ...
                if os.path.exists("alarm_server.lock"):
                    os.remove("alarm_server.lock")
            except:
                pass
    ...
    def setup_gui(self):
        ...
        # Server info frame
        server_info_frame = tk.Frame(status_frame, bg='#34495e')
        server_info_frame.pack(fill=tk.X, pady=(0, 5))
        
        # Server info
        server_info = tk.Label(server_info_frame, text=f"Host: {self.host} | Port: {self.port}", 
                              font=('Arial', 10), 
                              fg='#bdc3c7', bg='#34495e')
        server_info.pack(side=tk.LEFT)
        
        # Auto-detect IP button
        # self.auto_ip_btn = tk.Button(server_info_frame, text="Auto-Detect IP", 
        #                             command=self.auto_detect_ip,
        #                             bg='#3498db', fg='white', 
        #                             font=('Arial', 8, 'bold'))
        # self.auto_ip_btn.pack(side=tk.RIGHT, padx=(10, 0))
        ...
        # Handle window close - already set in __init__
    ...
    def write_audit_log(self, message, color, icon, name, sent_count, instructions=None):
        """Write alarm command to audit log file (logs.txt)"""
        try:
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            admin_name = self.current_admin if self.is_authenticated else "Unknown"
            floor = self.selected_floor if hasattr(self, 'selected_floor') else "Not Specified"
            
            log_entry = f"[{timestamp}] | Admin: {admin_name} | Alarm Type: {name} | "
            log_entry += f"Floor: {floor} | Message: {message} | "
            log_entry += f"Color: {color} | Icon: {icon} | "
            log_entry += f"Recipients: {sent_count} client(s)"
            
            if instructions:
                log_entry += f" | Instructions: {instructions}"
            ...
            self.log_message(f"Error writing to audit log: {e}", "error")
    ...
    def check_ip_availability(self):
        """Check if the configured IP is available on this system"""
        try:
            # Try to bind to the specific IP
            ...
            if e.errno == 10049:  # Windows: Address not available
                self.log_message(f"IP address {self.host} is not available on this system", "error")
                self.log_message("Available network interfaces:", "info")
                
                # Show available IPs
                import socket as socket_module
                hostname = socket_module.gethostname()
                local_ip = socket_module.gethostbyname(hostname)
                self.log_message(f"Hostname: {hostname}", "info")
                self.log_message(f"Local IP: {local_ip}", "info")
                ...
                try:
                    ...
                    if sys.platform == "win32":
                        ...
                        self.log_message("Network configuration:", "info")
                ...
                messagebox.showerror("IP Address Error", 
                                   f"IP address {self.host} is not available on this system.\n\n"
                ...
                self.log_message(f"Port {self.port} is already in use", "error")
                messagebox.showerror("Port Error", 
                                   f"Port {self.port} is already in use.\n\n"
                                   f"Another application might be using this port.\n"
                                   f"Try a different port or close the other application.")
                return False
            ...
            self.log_message(f"Unexpected error checking IP: {e}", "error")
            messagebox.showerror("Error", f"Unexpected error: {e}")
            return False
    
    def auto_detect_ip(self):
        """Auto-detect the best IP address to use"""
        try:
            import socket as socket_module
                ...
                if sys.platform == "win32":
                    ...
                    for line in result.stdout.split('\n'):
                        if 'IPv4' in line and '192.168.' in line:
                            ...
                            if ip and ip not in available_ips:
                                available_ips.append(ip)
                else:
                    # Linux/Mac
                    result = subprocess.run(['hostname', '-I'], capture_output=True, text=True)
                    for ip in result.stdout.split():
                        if ip and '192.168.' in ip and ip not in available_ips:
                            available_ips.append(ip)
            except:
                pass
            
            # Test each IP to see which one works
            working_ips = []
            for ip in available_ips:
                try:
                    test_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    test_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                    test_socket.bind((ip, self.port))
                    test_socket.close()
            ...
            if working_ips:
                ...
                new_ip = working_ips[0]
                old_ip = self.host
                self.host = new_ip
                
                # Update the display
                self.log_message(f"Auto-detected IP: {new_ip}", "success")
                self.log_message(f"Changed from {old_ip} to {new_ip}", "info")
                
                # Update the server info display
                for widget in self.root.winfo_children():
                    self.update_server_info_display(widget)
                
                messagebox.showinfo("IP Auto-Detected", 
                                 f"Auto-detected working IP: {new_ip}\n\n"
                                 f"Changed from: {old_ip}\n"
                                 f"You can now try starting the server.")
            else:
                self.log_message("No working IP addresses found", "error")
                messagebox.showerror("No IP Found", 
                                   "No working IP addresses found.\n\n"
                                   "Please check your network connection.")
                
        except Exception as e:
            self.log_message(f"Error auto-detecting IP: {e}", "error")
            messagebox.showerror("Auto-Detect Error", f"Error auto-detecting IP: {e}")
    
    def update_server_info_display(self, widget):
        """Recursively update server info display"""
        try:
            if isinstance(widget, tk.Label) and "Host:" in str(widget.cget("text")):
                widget.config(text=f"Host: {self.host} | Port: {self.port}")
            elif hasattr(widget, 'winfo_children'):
                for child in widget.winfo_children():
                    self.update_server_info_display(child)
        except:
            pass
    ...
    def broadcast_alarm(self, message, color, icon="⚠️", name="Custom Alarm"):
        ...
        if sent_count > 0:
            ...
            self.write_audit_log(message, color, icon, name, sent_count, instructions)
    ...
    def setup_system_tray(self):
        ...
        try:
            ...
            try:
                ...
                self.log_message("pystray not available - system tray disabled", "warning")
                self.tray_status_label.config(text="System Tray: Not Available", fg="#f39c12")
                return
            ...
            self.log_message("System tray is optional - server will work without it", "info")
            import traceback
    ...
    def hide_to_tray(self):
        ...
        if self.tray_icon:
            ...
            # If no system tray, just minimize to taskbar
            self.root.iconify()
            self.log_message("Server minimized to taskbar", "info")
